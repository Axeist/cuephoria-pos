...
export default async function handler(req: Request) {
  ...
  try {
    ...
    if (!merchantOrderId) {
      return j(
        { ok: false, error: "Missing merchantOrderId (or merchantTransactionId)" },
        400
      );
    }
    ...
    // Normalize state from PhonePe response
    // Expect top-level: { state: "COMPLETED"|"FAILED"|"PENDING", ... }
    const state =
      data?.state ||
      data?.data?.state ||
      data?.payload?.state ||
    ...
    return j({ ok: true, state, raw: data });
  } catch (err: any) {
