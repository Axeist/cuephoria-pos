...
async function oauthToken() {
  ...
  // PhonePe OAuth requires form-encoded body
  ...
  return { authz: `${type} ${token}`, raw: data };
}
...
export default async function handler(req: Request) {
  ...
  try {
    ...
    // Build server redirect endpoints that bounce back to SPA
    const siteOrigin =
      (process.env.NEXT_PUBLIC_SITE_URL
        ? new URL(process.env.NEXT_PUBLIC_SITE_URL).origin
        : undefined) || new URL(req.url).origin;

    const successRedirect = `${siteOrigin}/api/phonepe/return?status=success&order=${encodeURIComponent(
      orderId
    )}`;
    const failedRedirect = `${siteOrigin}/api/phonepe/return?status=failed&order=${encodeURIComponent(
      orderId
    )}`;
      ...
      return j(
        {
          ...
          status: r.status,
          body: data,
          error: "PhonePe pay response missing redirectUrl",
        },
        502
      );
    }
    ...
    return j(
      { ok: false, step: "exception", error: String(err?.message || err) },
      500
    );
  }
}
