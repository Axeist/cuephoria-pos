...
export default async function handler(req: Request) {
  ...
  try {
    ...
    const { data: rows, error: stError } = await supabase
      .from("stations")
      .select(
        "id, name, type, hourly_rate, team_name, team_color, max_capacity, single_rate, category, event_enabled, slot_duration, max_players, occupancy_rates, pricing_mode, duration_tiers, maintenance_mode"
      )
