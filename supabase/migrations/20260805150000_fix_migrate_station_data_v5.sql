-- Definitive migrate_station_data fix:
-- - Never DELETE bookings (cancel duplicates instead; preserves booking_views FK)
-- - Dedupe slot_blocks before repoint
-- - Fix invalid GET DIAGNOSTICS syntax that blocked prior migration from applying
...
BEGIN
  ...
  -- Cancel legacy live bookings that would double-book the target slot after repoint.
  UPDATE public.bookings b
  SET
    status = 'cancelled',
    status_updated_at = now(),
    status_updated_by = COALESCE(p_migrated_by, 'migrate_station_data'),
    notes = COALESCE(b.notes || E'\n', '') || '[auto] Cancelled during legacy station migration (duplicate slot)'
  ...
  GET DIAGNOSTICS v_bookings_cancelled = ROW_COUNT;
  ...
  SET
    ...
    status_updated_at = now(),
    status_updated_by = COALESCE(p_migrated_by, 'migrate_station_data'),
    notes = COALESCE(b.notes || E'\n', '') || '[auto] Cancelled during legacy station migration (duplicate among controllers)'
  FROM ranked r
  WHERE b.id = r.id
    AND r.rn > 1;
  GET DIAGNOSTICS v_cancelled_batch = ROW_COUNT;
  v_bookings_cancelled := v_bookings_cancelled + v_cancelled_batch;

  -- Repoint all bookings (including cancelled) to the consolidated station.
...
COMMENT ON FUNCTION public.migrate_station_data(UUID[], UUID, TEXT) IS
  'Legacy station migration v5: cancel (never delete) conflicting bookings, dedupe slot_blocks, repoint rows, delete legacy stations.';
