-- Fix migrate_station_data: cancel duplicate bookings instead of deleting them
-- (booking_views FK blocks hard deletes).
...
BEGIN
  ...
  -- Cancel (do not delete) legacy live bookings that would double-book the target slot.
  ...
  GET DIAGNOSTICS v_bookings_cancelled = v_bookings_cancelled + ROW_COUNT;
