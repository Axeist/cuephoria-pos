...
export default function PublicBooking() {
  ...
  const [selectedCustomerBooking, setSelectedCustomerBooking] = useState<CustomerBooking | null>(null);
  const [editingBooking, setEditingBooking] = useState<CustomerBooking | null>(null);
  const [newBookingTime, setNewBookingTime] = useState<TimeSlot | null>(null);
  ...
  const searchCustomerBookings = async () => {
    ...
    try {
      ...
      if (!bookings || bookings.length === 0) {
        toast.info("No upcoming bookings found for this customer");
        setCustomerBookings([]);
        return;
      }
      ...
      const bookingsWithStations: CustomerBooking[] = bookings.map(booking => ({
        ...booking,
        stationName: stationsData?.find(s => s.id === booking.station_id)?.name || "Unknown Station"
      }));

      setCustomerBookings(bookingsWithStations);
      setShowCustomerDetailsPopup(true);
      toast.success(`Found ${bookings.length} upcoming booking(s) for ${customer.name}`);

    } catch (error) {
      console.error("Error searching customer bookings:", error);
      toast.error("Failed to search customer bookings");
    } finally {
      setLoadingCustomerBookings(false);
    }
  };

  const updateBookingStatus = async (bookingId: string, newStatus: CustomerBooking['status']) => {
    try {
      ...
      // Refresh today's bookings if needed
      fetchTodaysBookings();
    } catch (error) {
  ...
  const startEditBooking = async (booking: CustomerBooking) => {
    setEditingBooking(booking);
    
    // Fetch available slots for the booking date and station
    try {
      const { data, error } = await supabase.rpc("get_available_slots", {
        p_date: booking.booking_date,
        p_station_id: booking.station_id,
        p_slot_duration: 60,
      });
      
      if (error) throw error;
      
      // Include the current booking slot as available
      ...
      const allSlots = data || [];
      const hasCurrentSlot = allSlots.some(slot => 
      ...
      setNewBookingTime({ start_time: booking.start_time, end_time: booking.end_time, is_available: true });
    } catch (error) {
  ...
  const saveBookingTimeChange = async () => {
    if (!editingBooking || !newBookingTime) return;

    // Check if time actually changed
    if (newBookingTime.start_time === editingBooking.start_time && 
        newBookingTime.end_time === editingBooking.end_time) {
      toast.info("No time change detected");
    ...
    try {
      ...
      const { data: conflicts } = await supabase
        .from("bookings")
        .select("id")
        .eq("station_id", editingBooking.station_id)
        .eq("booking_date", editingBooking.booking_date)
        .neq("id", editingBooking.id)
        .neq("status", "cancelled")
        .or(`start_time.lt.${newBookingTime.end_time},end_time.gt.${newBookingTime.start_time}`);

      if (conflicts && conflicts.length > 0) {
        toast.error("❌ Time slot conflict detected. Cannot change to this time.");
        return;
      }
      ...
      const { error } = await supabase
        .from("bookings")
        .update({
          start_time: newBookingTime.start_time,
          end_time: newBookingTime.end_time
        })
      ...
      setCustomerBookings(prev => 
        prev.map(booking => 
          booking.id === editingBooking.id 
            ? { ...booking, start_time: newBookingTime.start_time, end_time: newBookingTime.end_time }
            : booking
        )
      );
      ...
      <main className="px-4 sm:px-6 md:px-8 max-w-7xl mx-auto pb-14 relative z-10">
          ...
          <Card className="bg-white/5 backdrop-blur-xl border-white/10 rounded-2xl">
            <CardHeader>
              <CardTitle className="text-white flex items-center gap-2">
                <UserSearch className="h-5 w-5 text-cuephoria-lightpurple" />
                Manage Existing Booking
              </CardTitle>
              <p className="text-sm text-gray-400 mt-1">
                Search for your existing bookings using your phone number to cancel or modify them.
              </p>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex gap-2">
                <Input
                  value={manageBookingPhone}
                  onChange={(e) => setManageBookingPhone(e.target.value)}
                  placeholder="Enter customer phone number"
                  className="bg-black/30 border-white/10 text-white placeholder:text-gray-400 rounded-xl flex-1"
                />
                <Button
                  onClick={searchCustomerBookings}
                  disabled={loadingCustomerBookings}
                  className="rounded-xl bg-gradient-to-r from-cuephoria-blue to-cuephoria-purple"
                >
        ...
        {showCustomerDetailsPopup && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 p-4">
            <div className="max-h-[85vh] w-full max-w-4xl overflow-y-auto rounded-2xl border border-white/10 bg-[#0c0c13] p-6 shadow-2xl">
              <div className="mb-4 flex items-center justify-between">
                <h3 className="text-xl font-semibold text-white">Customer Bookings</h3>
                <button
                  aria-label="Close customer bookings"
                  onClick={() => {
                    ...
                    >
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="grid grid-cols-2 gap-4 text-sm">
                            <div>
                              ...
                              <p className="text-white">{format(new Date(booking.booking_date), "MMM d, yyyy")}</p>
                            </div>
                            <div>
                            ...
                            <div>
                              ...
                              <p className="text-white">{INR(booking.final_price)}</p>
                            </div>
                            <div>
                              <Label className="text-gray-400">Payment</Label>
                              <p className="text-white capitalize">{booking.payment_mode}</p>
                            </div>
                          </div>

                          {booking.coupon_code && (
                            <div>
                              <Label className="text-gray-400">Coupon Used</Label>
                              <p className="text-green-400">{booking.coupon_code}</p>
                            </div>
                          )}
                        </div>
                      ...
                      {editingBooking?.id === booking.id && (
                        <div className="mt-4 pt-4 border-t border-white/10">
                          <Label className="text-white font-medium">Change Time Slot</Label>
                          <div className="mt-2">
                            <TimeSlotPicker
                              slots={editAvailableSlots}
                              selectedSlot={newBookingTime}
                              onSlotSelect={setNewBookingTime}
                              loading={false}
                            />
                          </div>
