...
import type {
  ...
  CafeTable, CafeTableRow, CafeCartItem, CafeOrderRow,
} from '@/types/cafe.types';
import { transformMenuCategoryRow, transformMenuItemRow, transformTableRow } from '@/types/cafe.types';
import { Coffee, Plus, Minus, Trash2, ShoppingCart, User, Phone, MapPin, ArrowLeft, Loader2, Clock, CheckCircle2, ChefHat, Leaf } from 'lucide-react';
...
const CafeCustomerOrder: React.FC = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const locId = cafeLocation.data.id;

        const [catRes, itemRes, tableRes] = await Promise.all([
          supabase.from('cafe_menu_categories').select('*').eq('location_id', locId).eq('is_active', true).order('sort_order'),
          supabase.from('cafe_menu_items').select('*').eq('location_id', locId).eq('is_available', true).order('sort_order'),
          supabase.from('cafe_tables').select('*').eq('location_id', locId).eq('is_active', true).order('zone').order('sort_order'),
        ]);
  ...
  const filteredItems = displayCategory ? items.filter(i => i.categoryId === displayCategory) : items;
    ...
    <div className="min-h-screen bg-cuephoria-darker">
      ...
      <div className="sticky top-0 z-20 bg-[#1A1F2C] border-b border-gray-700/30 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          {step !== 'menu' && step !== 'tracking' && (
            <button onClick={() => setStep(step === 'checkout' ? 'cart' : 'menu')} className="p-1.5 rounded-lg hover:bg-gray-700/50">
                ...
                {zones.map(zone => (
                  <div key={zone}>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {tables.filter(t => t.zone === zone && !t.isOccupied).map(table => (
      ...
      {step === 'tracking' && (
        <div className="p-4 space-y-6 max-w-md mx-auto">
          ...
          <p className="text-[10px] text-gray-600 text-center font-quicksand">
