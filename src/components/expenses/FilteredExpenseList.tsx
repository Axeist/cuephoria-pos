...
interface FilteredExpenseListProps { startDate: Date; endDate: Date; }
...
const FilteredExpenseList: React.FC<FilteredExpenseListProps> = ({ startDate, endDate }) => {
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
        {Object.entries(categoryTotals).map(([category, amount]) => (
          <Card key={category} className="bg-gray-800 border-gray-700">
            ...
            <CardContent>
              <div className="text-xl font-bold text-white"><CurrencyDisplay amount={amount} /></div>
            </CardContent>
          </Card>
        ))}
        <Card className="bg-gray-800 border-2 border-purple-600">
          <CardHeader className="pb-2"><CardTitle className="text-sm font-medium text-purple-400">Total Expenses</CardTitle></CardHeader>
          <CardContent><div className="text-xl font-bold text-purple-400"><CurrencyDisplay amount={totalAmount} /></div></CardContent>
        </Card>
      </div>

      <Card className="bg-gray-800 border-gray-700">
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-gray-200">Expenses for Selected Period ({filteredExpenses.length} items)</CardTitle>
          <ExpenseDialog>
        ...
        <CardContent>
          {filteredExpenses.length === 0 ? (
            <div className="text-center py-8 text-gray-400">No expenses found for the selected period.</div>
          ) : (
                  ...
                  {filteredExpenses.map((expense) => {
                    ...
                    return (
                      <TableRow key={expense.id} className="border-gray-700 hover:bg-gray-700/50">
                        ...
                        <TableCell className="text-gray-200"><CurrencyDisplay amount={expense.amount} /></TableCell>
                        <TableCell className="text-gray-200">{format(new Date(expense.date), 'MMM dd, yyyy')}</TableCell>
                        <TableCell>
                          {expense.isRecurring ? (
                            ...
                            <AlertDialog>
                              ...
                              <AlertDialogContent className="bg-gray-800 border-gray-700">
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription className="text-gray-400">Are you sure you want to delete this expense? This action cannot be undone.</AlertDialogDescription>
                                </AlertDialogHeader>
                                <AlertDialogFooter>
                                  <AlertDialogCancel className="bg-gray-700 text-gray-200 hover:bg-gray-600">Cancel</AlertDialogCancel>
                                  <AlertDialogAction onClick={() => handleDeleteExpense(expense.id)} className="bg-red-500 hover:bg-red-600 text-white">Delete</AlertDialogAction>
                                </AlertDialogFooter>
                              </AlertDialogContent>
                            </AlertDialog>
                          </div>
