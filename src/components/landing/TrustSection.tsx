...
const METRICS = [
  { prefix: "", to: 99.98, decimals: 2, suffix: "%", label: "Uptime SLA", sub: "across all tenants" },
  { prefix: "<", to: 100, decimals: 0, suffix: "ms", label: "P95 API latency", sub: "globally served" },
  { prefix: "", to: 50, decimals: 0, suffix: "k+", label: "Bookings processed", sub: "and counting" },
  { prefix: "", to: 100, decimals: 0, suffix: "%", label: "Tenant isolation", sub: "row-level security" },
];
...
const CountMetric: React.FC<{
  ...
  decimals: number;
  suffix: string;
}> = ({ start, prefix, to, decimals, suffix }) => {
  const v = useCountUp({ to, decimals, duration: 1800, start });
  return (
    <span className="lp-mono text-3xl sm:text-4xl font-bold bg-gradient-to-r from-violet-300 via-fuchsia-300 to-cyan-300 bg-clip-text text-transparent tracking-tight">
      {prefix}
    ...
    <section id="trust" className="relative z-10 scroll-mt-32 py-24 sm:py-32 px-5 sm:px-8">
      ...
      <div className="max-w-7xl mx-auto relative">
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }}
          onViewportEnter={() => setMetricsIn(true)}
          variants={{ visible: { transition: { staggerChildren: 0.1 } } }}
          className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-20">
          {METRICS.map((m) => (
            <motion.div key={m.label} variants={fadeUp}
              className="lp-glass lp-scan p-5 text-center transition-colors hover:border-violet-500/30">
              <CountMetric
                start={metricsIn}
                prefix={m.prefix}
                to={m.to}
        ...
        <div className="grid grid-cols-12 gap-x-6 md:gap-x-10 mb-20">
          ...
          <motion.div className="col-span-12 lg:col-span-7 mb-12 lg:mb-0"
            ...
            <div className="relative rounded-3xl p-8 sm:p-10 overflow-hidden h-full"
              ...
              <div className="relative">
                <p className="lp-mono text-xs uppercase tracking-[0.22em] text-fuchsia-300 font-semibold mb-3">The Cuephoria Story</p>
                <h2 className="lp-display text-3xl sm:text-4xl font-bold tracking-tight mb-6 leading-tight">
                ...
                <div className="grid grid-cols-3 gap-3 mt-8">
                  {[{ v: "2 Years", l: "Live Ops" }, { v: "9 Modules", l: "In Production" }, { v: "100%", l: "Battle-Tested" }].map(s => (
                    <div key={s.l} className="rounded-xl p-4 text-center bg-white/[0.04] border border-white/10">
                      <div className="lp-mono text-xl font-bold bg-gradient-to-r from-violet-300 to-fuchsia-300 bg-clip-text text-transparent">{s.v}</div>
          ...
          <motion.div className="col-span-12 lg:col-span-5 flex flex-col gap-6"
            ...
            <div>
              <h3 className="lp-display text-2xl font-bold tracking-tight mb-5 text-white">Enterprise-grade security.</h3>
        ...
        <div className="mb-16">
          <motion.div className="text-center mb-10"
            ...
            <p className="lp-mono text-xs uppercase tracking-[0.22em] text-fuchsia-400 font-semibold mb-2">Operators love it</p>
            <h3 className="lp-display text-2xl sm:text-3xl font-bold tracking-tight">What venues are saying.</h3>
          </motion.div>
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }}
        ...
        <motion.div
          ...
          <div className="relative px-8 py-10 sm:py-12 flex flex-col sm:flex-row items-center justify-between gap-6">
            <div>
              ...
              <h3 className="lp-display text-2xl sm:text-3xl font-bold tracking-tight text-white">
            ...
            <div className="flex flex-col sm:flex-row gap-3 flex-shrink-0">
              <button type="button" onClick={() => navigate("/signup")}
                className="lp-btn h-12 px-7">
                Start free trial <ArrowRight size={16} />
              </button>
              <button type="button" onClick={() => document.getElementById("book-call")?.scrollIntoView({ behavior: "smooth" })}
                className="lp-btn-ghost h-12 px-7 text-sm">
                Book a demo call
              </button>
            </div>
          </div>
