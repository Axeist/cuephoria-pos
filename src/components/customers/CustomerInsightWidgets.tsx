...
const CustomerInsightWidgets: React.FC<CustomerInsightWidgetsProps> = ({ customers }) => {
  ...
  const recentCustomers = customers.filter(customer => 
    new Date(customer.createdAt) >= thirtyDaysAgo || customer.totalSpent > 0
  );
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 mb-6">
      {widgets.map((widget, index) => (
        <Card key={index} className="shadow-lg transition-all duration-300 hover:shadow-xl border-gray-200 dark:border-gray-700">
